...
history.back() // 回上一頁
history.forward() // 下一頁
widnow.history.go(0) // 重新整理
history.go() // same as widnow.history.go(0)
...
- [history.state](https://developer.mozilla.org/en-US/docs/Web/API/History/state): 下面會接著提到
...
- **title**: 對目前大多數的瀏覽且不刷器沒有任何作用。通常帶入 null 即可。
- **url**: 指定瀏覽器的 URL 顯示
