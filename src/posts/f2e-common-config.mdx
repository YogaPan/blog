...
date: '2021-07-11'
tags: ['javascript', 'lerna', 'github workflows', 'npm']
description: '介紹如何將前端常用的 eslint, prettier, browserslist 設定提取至 Lerna 管理的 monorepo，並利用 Github Actions 自動發布至 npm。這篇文章也記錄了如何利用 npm link 在本機快速測試套件。'
---
...
```json
{
  "extends": ["eslint:recommended"],  // extends 官方推薦的規則，讓你不必每條規則逐一設定
  "rules": {
    "no-console": "error",  // 假設我認為 debug 用的 console 忘記拿掉的人都該死，我就加這條規則。
    "no-sparse-arrays": "off" // 假設我認為官方推薦的規則太嚴格了，那我就可以 override 這條規則把檢查關掉。
  }
}
```
...
```json
{
  "extends": "@my-org/eslint-config"  // 自己吃自己，自己訂定的規則自己也必須遵守。
}
```
...
### 2.5 使用 npm link 快速試用

在開發初期，如果需要在其他專案下引入這份設定一邊試用一邊進行調整的話，那麼每次修改完 `@my-org/eslint-config` 都需要發布至 npm 後再 install 下來也太沒效率了。這個時候就會需要借助 [npm link](https://docs.npmjs.com/cli/v7/commands/npm-link) 的力量了。
...
在我們需要引入 `eslint-config` 進行測試的專案執行 `npm link @my-org/eslint-config` 建立連結，設定完後我們就能直接修改 `@my-org/eslint-config` 的內容。因為是軟連結，修改會即時反應在所有其他執行過 `npm link @my-org/eslint-config` 的專案下，以測試的角度來說相當方便。
...
[Lerna](https://github.com/lerna/lerna) 就是用來管理這些小王八蛋的，透過指令協助開發者對多個模組進行管理。使用方式很簡單，在一個新資料夾底下執行 `lerna init --independent`。會產生 `package.json`, `lerna,json` 以及 `packages/`
...
我們可以看到我們成功創立了一個 independent mode 的 monorepo，並且 `packages/*` 就是這些小王八蛋的住所。接下來就是把各模組放到裏面，架構會長得像是下面這樣：
...
## 4. Github Actions 自動 Publish

能自動化的事情就盡量自動化，開發者會希望能夠專注在開發上，publish 就交給自動化的流程去處理。[Github Actions](https://github.com/features/actions) 可以協助我們在每次 push 程式碼到 Github 上後，自動幫我執行 build, test, lint 一系列動作，若都成功就 publish 至 npm。

### 4.1 設定 npm Automation Token

在本機上必須先用帳號密碼登入 npm 才有權限 publish，那如何賦予 Github Actions 權限呢？方法是在 npm 上面產生一組 automation token ，接著在 Github repo 上的 settings -> secrets 設定 `NPM_TOKEN` 的值。有了這把 token 就能夠任意發布套件至你個人或是 organization 底下，所以必須小心保管。

註：npm token 有三種，但有 publish 權限且可繞過 2FA 的只有 automation token。
